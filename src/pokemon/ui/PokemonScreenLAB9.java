...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		currentPosx = (ran.nextInt(W)/40)*40; // this should be a random position
		currentPosy = (ran.nextInt(H)/40)*40; // this should be a random position
		
		pikachu = new ImageView(new File("icons/pikachu.gif").toURI().toString());
	...
	private void moveAvatar(double x, double y) {
		...
		if (x - cx >= 0 && x + cx <= W && y - cy >= 0 && y + cy <= H) {
			...
			pikachu.relocate(avatar.getLayoutX(), avatar.getLayoutY());
